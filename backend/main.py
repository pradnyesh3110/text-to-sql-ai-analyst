...
from backend.database import get_db, User, QueryLog
...
# ─── Query Route (THE IMPORTANT ONE) ──────────────────
@app.post("/generate-sql")
def generate_sql(req: QueryRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
@app.get("/")
def root():
    return {"status": "Text-to-SQL API running", "mode": "demo" if __import__('os').environ.get("DEMO_MODE") == "true" else "local"}
import os
import shutil
import traceback
